...
void generate_txt_rec(int i){
        ...
        while(ptr!=NULL){
                ...
                sprintf(add_txt_rec[n++],"T%06X02%4X\n",ptr->address,s[i]->head->address);
                printf("%s\n",add_txt_rec[n-1]);//1
                free(ptr);
                ptr=temp;
        }
...
void one_pass(){
        ...
        char text_record[100],mach_inst[30],pgm_name[30];
        int loc_ctr=0,start_addr,t_size=30,prev_start_addr;
        int op_found,t_len=0,length;
        pgm_name[0]='\0';
